...
## Main statements
 ...
 - `dE_int_Asymptotic_coprime` shows the asymptotic behavior of `dE_int` (integer version of `dE`)
   when $s$ and $t$ are coprime.
...
/-!
Analog to `φδₖ`, `Φ` sends `δₖ` back to `nₖ`.
But since `Φ` is discrete, we can develop another theorem:
when `Φ` changes, it precisely correspond to when `nₖ` move to the next value.
-/
...
/-!
We will investigate the generator function $Z\{Φ\}(x) = \sum_{i\inℕ} Φ(i) x^i$ and
...
We also show `ξPolynomial` doesn't have multiple root and has simple factorization over `ξSet`.
-/
noncomputable
...
/-!
The generating function Z{Φ}(x) converges to a rational function.
...
/-!
With formula for `Φ` developed, we further show one of the term in its summation
...
/-!
`ξ₀` Is the "main term" in `ξSet`.
It is a real number between 0 and 1,
and has the smallest norm among `ξSet` when $s$ an $t$ are coprime.
-/
noncomputable
...
/-!
As the "inverse function" of `Φ`, `dE` grows like `log`.
...
/-!
`wₖ` grows linearly w.r.t. `nₖ` for integer $s$ and $t$.
We can then conclude that `wₗᵢ`, as the linear interpolation, grows at the same rate.
-/
